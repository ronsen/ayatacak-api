export const translateVerse = (verse) => {
    ...
    const regex = /(\d*)\s*([a-z]+)\s*(\d+)(?::(\d+))?(\s*-\s*(\d+)(?:\s*([a-z]+)\s*(\d+))?(?::(\d+))?)?/i;

    let match = verse.match(regex);
    let book = (match[1] + ' ' + match[2]).trim();
    let index = books.findIndex((b) => b.toLowerCase() == book.toLowerCase())

    let value = buku[index] + ' ' + match[3] + ':' + match[4];
